...
function createUnconfiguredRepository(missing: readonly string[]): OperationsRepository {
  ...
  return {
    ...
    addDemoUnassignedLoad: fail,
    assignShipment: fail,
    assignVehicle: fail,
    createCustomerRequest: fail,
    createMaintenanceOrder: fail,
    getShipmentEdiTransactions: () => [],
    getState: () => state,
    hydrate: fail,
    issuePayout: fail,
    listPayoutMethods: fail,
    markMessageRead: fail,
    markPayoutPaid: fail,
    removeAvailabilityBlock: fail,
    removeAvailabilityRule: fail,
    removeDriverShift: fail,
    removePayoutMethod: fail,
    reportException: fail,
    resetDemo: fail,
    resolveException: fail,
    respondToTender: fail,
    respondToShiftCoverage: fail,
    requestShiftCoverage: fail,
    retryScheduleSync: fail,
    sendMessage: fail,
    setAvailabilityBlock: fail,
    setAvailabilityRule: fail,
    setDriverShift: fail,
    setDefaultPayoutMethod: fail,
    signIn: fail,
    signOut: fail,
    recordDriverLocation: fail,
    savePayoutMethod: fail,
