...
export default function JobAssignmentScreen() {
  ...
  const submit = async () => {
    ...
    const offerPriceCents = Math.round((price === "Custom" ? Number(customPrice) : price) * 100);
    if (ready) ready = await actions.assignShipment(shipment.id, driverId, offerPriceCents);
