...
/**
 * The hero is a permanently dark panel, so its labels cannot come from the
 * active appearance: in light mode `theme.text` is pure black and would be
 * invisible against the gradient. These are the dark theme's own label colours.
 */
const HERO_ON_DARK = { title: "#FFFFFF", body: "#C7C7CC", accent: "#E8F060" } as const;

function CustomerHero({ name, onRequest }: { readonly name: string; readonly onRequest: () => void }) {
  const theme = useTheme();
  return <LinearGradient colors={["#171A12", "#0C0E0A"]} style={[styles.customerHero, { borderBottomColor: theme.border }]}><View accessibilityElementsHidden style={[styles.heroArtCircle, { backgroundColor: theme.surfaceBright, borderColor: theme.borderLight }]}><Image resizeMode="cover" source={customerArt} style={styles.heroArt} /></View><Text style={[styles.heroEyebrow, { color: HERO_ON_DARK.accent }]}>WELCOME BACK, {name.toUpperCase()}</Text><Text style={[styles.customerHeroTitle, { color: HERO_ON_DARK.title }]}>Freight Delivery,{"\n"}<Text style={{ color: HERO_ON_DARK.accent }}>Simplified.</Text></Text><Text style={[styles.customerHeroDescription, { color: HERO_ON_DARK.body }]}>Reliable freight capacity and local operations—without the runaround.</Text><AnimatedPressable accessibilityRole="button" haptic="selection" onPress={onRequest} style={[styles.customerHeroButton, { backgroundColor: theme.primary }]}><Text style={[styles.customerHeroButtonText, { color: theme.primaryForeground }]}>Request Freight</Text><Feather color={theme.primaryForeground} name="arrow-up-right" size={18} /></AnimatedPressable></LinearGradient>;
}
