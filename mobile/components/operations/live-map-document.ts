...
export function buildMapHtml(bottomInsetRatio: number, glideMs: number): string {
...
<script>
  var post = function (msg) {
    ...
    else window.parent.postMessage({ source: "mf-fleet-map", ...msg }, "*");
  };
  var GLIDE = ${glideMs};
  var TRUCK = ${truck};
  var HOME = ${bounds};
  ...
  window.addEventListener('message', function (event) {
    if (event.source !== window.parent || !event.data || event.data.source !== 'mf-fleet-parent') return;
    var data = event.data;
    window.__setMeta(data.meta);
    window.__setPositions(data.positions, data.selectedId);
    if (data.focus) window.__focus(data.focus.longitude, data.focus.latitude);
  });
